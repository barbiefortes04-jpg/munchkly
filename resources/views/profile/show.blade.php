            ...
            <div class="relative px-8 pb-8">
                    ...
                    @if($isAuth && $authUser && (is_array($authUser) ? $authUser['id'] : $authUser->id) != $user->id)
                        <div class="flex space-x-3 justify-center">
                            <button onclick="toggleFollow({{ $user->id }})" 
                                    class="follow-btn px-8 py-3 rounded-full font-medium transition-all transform hover:scale-105 {{ $isFollowing ? 'bg-gray-100 hover:bg-gray-200 text-gray-700 border border-gray-300' : 'bg-gradient-to-r from-blue-500 to-purple-600 hover:from-blue-600 hover:to-purple-700 text-white shadow-lg' }}"
                            ...
                            @if($isFollowing)
                                <button onclick="toggleCloseFriend({{ $user->id }})" 
                                        class="close-friend-btn px-6 py-3 rounded-full font-medium transition-all transform hover:scale-105 {{ $isCloseFriend ? 'bg-green-100 hover:bg-green-200 text-green-700 border border-green-300' : 'bg-white hover:bg-gray-50 text-gray-700 border border-gray-300' }}"
                                        id="close-friend-btn-{{ $user->id }}">
                                    <i class="fas {{ $isCloseFriend ? 'fa-heart' : 'fa-heart-o' }} mr-2" style="color: {{ $isCloseFriend ? '#10B981' : '#6B7280' }}"></i>
                ...
                <div class="grid grid-cols-4 gap-6 p-6 bg-gray-50 rounded-xl">
                    <div class="text-center">
                        <div class="text-2xl font-bold text-blue-600">{{ $tweetCount }}</div>
                        <div class="text-sm text-gray-600 font-medium">{{ $tweetCount === 1 ? 'Tweet' : 'Tweets' }}</div>
                    </div>
                    <div class="text-center">
                        <div class="text-2xl font-bold text-red-500">{{ $totalLikesReceived }}</div>
                        <div class="text-sm text-gray-600 font-medium">{{ $totalLikesReceived === 1 ? 'Like' : 'Likes' }}</div>
                    </div>
                    <div class="text-center cursor-pointer hover:bg-gray-100 rounded-lg p-2 transition-colors" onclick="toggleFollowersModal()">
                        ...
                        <div class="text-sm text-gray-600 font-medium hover:text-blue-600">{{ $followersCount === 1 ? 'Follower' : 'Followers' }}</div>
                    </div>
                    <div class="text-center">
