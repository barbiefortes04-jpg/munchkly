                        ...
                        <div class="bg-white rounded-lg shadow p-4">
                            <div class="flex items-start space-x-3">
                                ...
                                <div class="flex-shrink-0">
                                    @if(isset($tweet->user->profile_picture) && $tweet->user->profile_picture)
                                        <img class="h-10 w-10 rounded-full object-cover" src="{{ Storage::url('profile_pictures/' . $tweet->user->profile_picture) }}" alt="{{ $tweet->user->name }}">
                                    @else
                                        <div class="h-10 w-10 rounded-full bg-blue-500 flex items-center justify-center">
                                            <span class="text-white font-medium text-sm">{{ strtoupper(substr($tweet->user->name, 0, 1)) }}</span>
                                        </div>
                                    @endif
                                </div>
                                ...
                                <div class="flex-1 min-w-0">
                                                                    ...
                                                                    <div class="relative">
                                                                        ...
                                                                        @if($index == 3 && count($tweet->photos) > 4)
                                                                            <div class="absolute inset-0 bg-black bg-opacity-50 rounded flex items-center justify-center">
                                                                                <span class="text-white font-bold text-sm">+{{ count($tweet->photos) - 4 }}</span>
                                                                            </div>
                                                                        @endif
                                                                    </div>
                                                                @endif
                                    ...
                                    <div class="flex items-center space-x-4 text-gray-500">
                                        ...
                                        @if($isAuth)
                                            ...
                                            >
                                                <svg class="h-5 w-5 {{ $tweet->user_has_liked ? 'text-red-500 fill-current' : '' }}" fill="{{ $tweet->user_has_liked ? 'currentColor' : 'none' }}" stroke="currentColor" viewBox="0 0 24 24">
                                        ...
                                        @if($isAuth && $authUser && (is_array($authUser) ? $authUser['id'] : $authUser->id) == $tweet->user->id)
                                            <a href="{{ route('tweets.edit', $tweet->id) }}" class="text-blue-500 hover:text-blue-700 text-sm">
                                                Edit
                                            </a>
                                            <button 
                                                onclick="deleteTweet({{ $tweet->id }}, this)"
                            ...
                            <div class="flex items-center space-x-4">
                                ...
                                <div class="flex-shrink-0">
                                    @if(isset($user->profile_picture) && $user->profile_picture)
                                        <img class="h-12 w-12 rounded-full object-cover" src="{{ Storage::url('profile_pictures/' . $user->profile_picture) }}" alt="{{ $user->name }}">
                                    @else
                                ...
                                <div>
                                    <a href="{{ route('profile.show', $user->id) }}" class="bg-blue-500 text-white px-4 py-2 rounded-full hover:bg-blue-600 transition-colors">
                                        View Profile
                                    </a>
                                </div>
                            </div>
